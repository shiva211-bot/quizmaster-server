...
const table='ALTER TABLE users ADD COLUMN IF NOT EXISTS phone TEXT';
if(!s.includes(table)){
  const marker='await pool.query("ALTER TABLE users ADD COLUMN IF NOT EXISTS class_level INTEGER");';
  if(s.includes(marker))s=s.replace(marker,marker+table+';');
}
...
if(s.includes(oldReq))s=s.replace(oldReq,neuReq);
...
if(s.includes(oldInsert))s=s.replace(oldInsert,newInsert);
...
if(s.includes(oldMe))s=s.replace(oldMe,newMe);
