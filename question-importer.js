...
const marker='/* QUIZMASTER_QUESTION_IMPORTER_V4 */';
...
function __qmO(v){let m=v.match(/^\\s*\\(?([A-Da-d])\\)?[.)]?\\s*(.*)$/);if(m&&m[2])return [m[1].toUpperCase(),m[2]];m=v.match(/^\\s*([1-4])[.)]\\s*(.*)$/);return m&&m[2]?[String.fromCharCode(64+Number(m[1])),m[2]]:null}
...
function __qmParse(raw){
 ...
 qs.forEach((x,i)=>{const miss=['A','B','C','D'].filter(k=>!x.o[k]);if(!x.text)errors.push('Question '+(i+1)+' has no question text');const multi=/select\\s+all\\s+possible\\s+options?/i.test(x.text);if(miss.length>0&&!(miss.length===2&&x.o.A&&x.o.B&&/^True$|^False$/i.test(x.o.A)&&/^True$|^False$/i.test(x.o.B)))errors.push('Question '+(i+1)+' is missing option(s): '+miss.join(', '));if(multi)errors.push('Question '+(i+1)+' is a multi-select question; single-answer import is required.');if(!x.answer&&uniqueKeys.has(x.number))x.answer=uniqueKeys.get(x.number);if(!x.answer)warnings.push('Question '+(i+1)+' has no answer-key entry');if(x.answer&&!x.o[x.answer])errors.push('Question '+(i+1)+' answer key points to missing option '+x.answer);if(x.text&&!miss.length)good.push(x);else if(x.text&&miss.length===2&&x.o.A&&x.o.B&&/^True$|^False$/i.test(x.o.A)&&/^True$|^False$/i.test(x.o.B)&&x.answer)good.push(x)});
